...
<div id="kt_content_container" class=" container-xxl ">
    ...
    <div class="row">
                      ...
                      <tr>
                        <td class="ps-0">
                          <div class="d-flex align-items-center gap-3 fw-semibold">
                            <img src="{{ getImage(getFilePath('userProfile') . '/' . @$data->customer->image, getFileSize('userProfile')) }}" class="rounded-circle" alt="user"
                              width="45" /> {{@$data->customer->username}}
                          </div>
                        </td>
                        <td>
                          <span>{!! date(' D d, M Y', strtotime($data->created_at)) !!}<br><small>
                            {{ date('h:i A', strtotime($data->created_at)) }}</small></span>
                        </td>
                        <td class="text-success">{{$general->cur_sym}}{{number_format($data->amount,2)}}</td>
                      </tr>
                      @empty
                      {!!emptyData2()!!}
                      @endforelse
        ...
        <div class="col-xl-4 d-flex align-items-strech">
          <div class="card w-100">
            <img src="{{QR($url)}}" class="card-img" alt="nft" />
            <div class="p-4 mt-n4 text-center">
              <div class="position-relative mt-n4">
                <img src="{{ getImage(getFilePath('userProfile') . '/' . auth()->user()->image, getFileSize('userProfile')) }}" alt="nft"
                  class="rounded-circle border border-3 border-white" width="50" />
              </div>
              <div>
                <h6 class="mb-0 fw-semibold mt-2">{{auth()->user()->fullname}}</h6>
              </div>
              <div class="d-flex align-items-center justify-content-between mt-2 pb-3 border-bottom">
                <div class="text-start">
                  ...
                  <h6 class="mb-0 fw-semibold text-danger">{{$general->cur_sym}}{{number_format($payment,2)}}</h6>
                </div>
                <div class="text-end">
                  <span class="fs-6">Total Received</span>
                  <h6 class="mb-0 fw-semibold text-success">{{$general->cur_sym}}{{number_format($received,2)}}</h6>
                </div>
              </div>
