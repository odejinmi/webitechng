                                            ...
                                            <select name="country" class="form-select form-select-solid"
                                                ...
                                                @foreach ($countries as $data)
                                                    <option data-countryname="{{ $data['name'] }}"
                                                        ...
                                                        data-countrycontinent="{{ $data['continent'] }}"
                                                        value="{{ $data['isoName'] }}"
                                        ...
                                        @push('script')
                                            <script>
                                                ...
                                                `;
                                                    ...
                                                    $.ajax({
                                                        url: "{{ route('user.internet.operators') }}",
                                                        type: 'GET',
                                                        async: true,
                                                        data: {
                                                            _token: _token,
                                                        ...
                                                        success: function(data) {
                                                            if (data.status == 'true') {
                                                                ...
                                                                plans.map(plan => {
                                                                    ...
                                                                    if(plan['denominationType'] == 'FIXED')
                                                                    {
                                                                    ...
                                                                    `<label class="d-flex flex-stack cursor-pointer mb-5" for="${plan['operatorId']}" >
                                                                        ...
                                                                        <span class="form-check form-check-custom form-check-solid">
                                                                            <input class="form-check-input" type="radio" onchange="networkprovider('${plan['destinationCurrencySymbol']}','${plan['logoUrls'][0]}','${JSON.stringify(fixedAmounts)}','${countryCode}','${operatorId}','${provider}','${rate}','${currency}','${min}','${max}')"
                                            ...
                                            <script>
                                              function networkprovider(symbol,image,arrayamounts,countryCode,operatorId,network,rate,currency,minAmount,maxAmount) {
                                                        ...
                                                        getOperation(operatorId,symbol,image);
                                                    }

                                                    //GET OPERATORBY ID
                                                    function getOperation(operatorId,currency,image)
                                                            {
                                                                ...
                                                                fetch("{{ route('user.internet.operatorsInternetdetails') }}", requestOptions)
                                                                ...
                                                                .then(result =>
                                                                {
                                                                    ...
                                                                    //console.info(typeof JSON.parse(result))
                                                                    const courses = JSON.parse(result);
                                                                    const plans = Object(courses);
                                                                    for (const key in plans) {

                                                                   // console.log(`${key}: ${plans[key]}`);

                                                                    let htmlSegment = `
                                                                    <label class="d-flex flex-stack cursor-pointer mb-5" for="${key}" >
                                                                        <span class="d-flex align-items-center me-2">
                                                                            <span class="symbol symbol-50px me-6">
                                                                                <span class="symbol-label bg-light-primary">
                                                                            ...
                                                                            <span class="d-flex flex-column">
                                                                                <span class="fw-bold fs-6">${plans[key]}</span>
                                                                                <span class="fs-7 text-muted">${currency}${key}</span>
                                                                            </span>
                                                                        </span>

                                                                        <span class="form-check form-check-custom form-check-solid">
                                                                            <input class="form-check-input"  onchange="setamount(this)" type="radio"
                                                                                name="operator" id="${key}" value="${key}|${plans[key]}" />
                                                                        </span>
                                                                    </label>
                                                                        `;
                                                                        html += htmlSegment;
                                            ...
                                            @push('script')
                                                <script>
                                                    function setamount(input) {
                                                        ...
                                                        document.getElementById("dataplan").innerHTML = input.value.split('|')[1];
                                                        document.getElementById("subtotalamount").innerHTML = amount+currency;
                                                        document.getElementById("totalamount").innerHTML = total.toFixed(2)+'{{$general->cur_text}}';
                                                        document.getElementById("rate").innerHTML = '1{{$general->cur_text}} = '+rate+currency;
                                                    }
                                                </script>
                                            @endpush
                                                              ...
                                                              <td class="text-muted">
                                                                  <div class="d-flex align-items-center">
                                                                      <i class="ti ti-wifi fs-2 me-2"><span
                                                                              ...
                                                                              class="path3"></span><span
                                                                              class="path4"></span><span
                                                                              class="path5"></span></i> @lang('Operator')
                                                              ...
                                                              <td class="text-muted">
                                                                  <div class="d-flex align-items-center">
                                                                      <i class="ti ti-exchange fs-2 me-2"><span
                                                                              class="path1"></span><span
                                                                              class="path2"></span></i> @lang('Rate')


                                                                      <span class="ms-1" data-bs-toggle="tooltip"
                                                                          title="This is the payment exchange rate">
                                                                          <i
                                                                              class="ti ti-alert-circle  text-gray-500 fs-6"><span
                                                                                  class="path1"></span><span
                                                                                  class="path2"></span><span
                                        ...
                                        <div class="pb-8 pb-lg-10">
                                            ...
                                            <section class="payment-method text-center">
                                              ...
                                              <div class="d-sm-flex align-items-center justify-content-between my-4">
                                               <center>  <a href="{{ route('user.internet.history') }}" class="btn btn-primary d-block">@lang('View Order')</a> </center>
                                              </div>
                                            </section>
                                        </div>
                                    </div>
                                        ...
                                        function() {
                                            ...
                                            function submitform()
                                            {
                                              ...
                                              var raw = JSON.stringify({
                                                ...
                                                fetch("{{ route('user.buy.internet') }}", requestOptions)
                                                  .then(response => response.text())
                                                  .then(result =>
                                                  {
                                                    resp = JSON.parse(result);
