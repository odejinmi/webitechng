                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                           <small>{{ __($general->cur_text) }} {{ showAmount($deposit->payment / $deposit->quantity) }}</small>
                                            <br>
                                            <strong>{{ showAmount($deposit->price) }} {{ __($deposit->currency) }}</strong>                                        </td>
                                        <td class="text-center">
                                           <label class='badge @if($deposit->status == 'delivered') bg-success @elseif($deposit->status == 'delivered') bg-success  @elseif($deposit->status == 'PENDING') bg-warning  @else  bg-danger @endif'> @php echo $deposit->status @endphp</label>
                                        </td>
                                    </tr>
                                @empty
                                    {!!emptyData2()!!}
