...
class RegisterController extends Controller
{
    ...
    public function showRegistrationForm($ref = null)
    {
        ...
        $reference = $ref;
        if (isset($reference)) {
            $exist = User::where('username', $reference)->first();
            if(!$exist)
            {
            $notify[] = ['error', 'There is no user with this referal link'];
            return to_route('user.register')->withNotify($notify);
            }
    ...
    public function register(Request $request)
    {
        ...
        if (isset($request->referBy)) {
            $exist = User::where('username', $request->referBy)->first();
            if(!$exist)
            {
              $notify[] = ['error', 'A user with this referral link does not exists'];
              return back()->withNotify($notify)->withInput();
            }
