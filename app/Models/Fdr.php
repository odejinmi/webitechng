...
class Fdr extends Model
{
    ...
    {
        return $this->profit ?? 1 / $this->per_installment;
    }
    ...
    public function statusBadge(): Attribute
    {
        return Attribute::make(get: function () {
            if ($this->status == 1 && $this->next_installment_date < today()) {
                return '<span class="badge bg-warning">' . trans('Due') . '</span>';
            } elseif ($this->status == 1) {
                return '<span class="badge bg--success">' . trans('Running') . '</span>';
            } else {
                return '<span class="badge bg-dark">' . trans('Closed') . '</span>';
            }
        });
    }
}
