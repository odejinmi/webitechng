...
class Deposit extends Model
{
    ...
    public function badgeData()
    {
        ...
        if ($this->status == Status::PAYMENT_PENDING) {
            ...
            $html = '<span><span class="badge bg-success">' . trans('Approved') . '</span><br>' . diffForHumans($this->updated_at) . '</span>';
        } elseif ($this->status == Status::PAYMENT_SUCCESS && $this->method_code < 1000) {
            $html = '<span class="badge bg-success">' . trans('Succeed') . '</span>';
        } elseif ($this->status == Status::PAYMENT_REJECT) {
            $html = '<span><span class="badge bg-danger">' . trans('Rejected') . '</span><br>' . diffForHumans($this->updated_at) . '</span>';
        } else {
            $html = '<span><span class="badge bg-dark">' . trans('Initiated') . '</span></span>';
        }
        return $html;
    }
